...
def production_status() -> dict:
    ...
    checks = {
        ...
        "postgres_database": db.startswith("postgresql+") or db.startswith("postgresql://"),
    ...
    if cloudflare_deployment:
        ...
        checks.update({
            ...
            "cloudflare_api_host": "api.ragab-seddik.com" in allowed_hosts or (public.startswith("https://staging-api.") and (urlparse(public).hostname or "") in allowed_hosts),
            "separated_frontend_cors": bool(frontend_parsed and frontend_parsed.scheme == "https" and frontend_parsed.hostname and frontend_origin in frontend_origins),
        })
    optional = {
    ...
    return {"required": checks, "integrations": optional, "required_ok": required_ok, "full_launch_ok": required_ok and all(optional.values())}
