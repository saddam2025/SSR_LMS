...
def admin_user_create(request: Request, name: str = Form(...), email: str = Form(""), phone: str = Form(""), role: str = Form("student"), password: str = Form(...), csrf: str = Form(...), db: Session = Depends(get_db)):
    ...
    if role == "student":
        # Students are identified by phone number, not email. A unique
        # placeholder email is generated automatically to satisfy the
        # database's email column (still unique/required), matching what
        # public self-registration already does.
        ph = re.sub(r"\D", "", phone or "")
        if ph.startswith("20") and len(ph) == 12: ph = "0" + ph[2:]
        if len(ph) == 10 and ph.startswith("1"): ph = "0" + ph
        if not re.fullmatch(r"01[0-9]{9}", ph): raise HTTPException(400, "رقم الهاتف غير صالح. مثال: 01xxxxxxxxx")
        if db.query(StudentProfile).filter(StudentProfile.phone == ph).first(): raise HTTPException(409, "رقم الهاتف مستخدم بالفعل")
        placeholder_email = f"{ph}@mostashar.local"
        target = User(name=name.strip()[:120], email=placeholder_email, password_hash=hash_password(password), role=role, is_active=True)
        db.add(target); db.flush()
        db.add(StudentProfile(user_id=target.id, phone=ph))
    else:
        email_norm = email.strip().lower()
        if not re.fullmatch(r"[^@\s]+@[^@\s]+\.[^@\s]+", email_norm): raise HTTPException(400, "البريد الإلكتروني غير صالح")
        if db.query(User).filter(func.lower(User.email) == email_norm).first(): raise HTTPException(409, "البريد مستخدم بالفعل")
        target = User(name=name.strip()[:120], email=email_norm, password_hash=hash_password(password), role=role, is_active=True)
        db.add(target)
    db.commit(); audit(db, request, u, "user_created", {"target_user": target.id, "role": role})
