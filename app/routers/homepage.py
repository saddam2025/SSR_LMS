...
from ..models import Course, HomepageFeature, HomepageReel, HomepageHonor
...
def home(request: Request, db: Session = Depends(get_db)):
    ...
    if u:
        context = ctx(request, db, courses=courses, reels=reels, honors=honors, reels_enabled=reels_enabled, honors_enabled=honors_enabled)
    else:
        context = {
            ...
            "courses": courses, "reels": reels, "honors": honors,
            "reels_enabled": reels_enabled, "honors_enabled": honors_enabled,
        }
...
def admin_homepage(request: Request, db: Session = Depends(get_db)):
    ...
    return render_template("admin_homepage.html", ctx(request, db, reels=reels, honors=honors,
        reels_enabled=feature_enabled(db, "reels", True), honors_enabled=feature_enabled(db, "honors", True)))

@router.post("/admin/homepage/features")
def admin_homepage_features(request: Request, reels_enabled: str = Form(""), honors_enabled: str = Form(""), csrf: str = Form(...), db: Session = Depends(get_db)):
    u = require_role(request, db, "admin")
    if not check_csrf(request.session, csrf): raise HTTPException(403)
    for key, enabled in (("reels", bool(reels_enabled)), ("honors", bool(honors_enabled))):
    ...
    db.commit(); audit(db, request, u, "homepage_features_updated", {"reels": bool(reels_enabled), "honors": bool(honors_enabled)})
